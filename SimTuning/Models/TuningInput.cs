//using System;
//using System.Collections.Generic;
//using System.Text;

//namespace SimTuning.Models
//{
//    public class TuningInput
//    {
        //public double? ZylinderZ { get; set; }
        //public double? HubL { get; set; }
        //public double? BohrungD { get; set; }
        //public double? PleulL { get; set; }
        //public double? VerdichtungV { get; set; }
        //public double? KanalausarbeitungF { get; set; }
        //public double? UeberstroemerSW { get; set; }
        //public double? EinlassKanalL { get; set; }
        //public double? VergaserD { get; set; }

        //public double? VenturiF { get; set; }
        //public double? AbgasV { get; set; }

        //public double? EinlassSW { get; set; }
        //public double? EinlassD { get; set; }
        //public double? AbgasT { get; set; }
        //public double? AuslassSW { get; set; }
        //public double? AuslassD { get; set; }
        //public double? AuslassResoD { get; set; }
        
        
        //public double? HeizwertU { get; set; }
        
        //public double? UmgebungP { get; set; }
        //public double? UmgebungT { get; set; }
//    }
//}